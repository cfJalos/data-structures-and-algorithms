...
const createServer = () => {
  ...
  app.get('/aboutme', (request,response) => {
    let bio = {name:'czarl', lastname:'jalos',};
    response.status(200).send(bio);
  });

  app.get('/favoritefoods', (request,response) => {
    let myFood = {breakfast:'eggs',lunch:'chicken',dinner:'beef'};
    response.status(200).send(myFood);
  });
...
const count = (target, input) => {
  // Solution code here...
  let num = 0;
  let results = input.map(data => {
    const array =  data.filter(entry => {
      if (entry == target) {
        return num++;
      }
    });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let results = input.map(data => data.filter(entry => entry % 5 === 0 && typeof entry === 'number').map(entry => Math.pow(2,entry)));
